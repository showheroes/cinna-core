...
| `routing_message_text_gating_test.py` | `ROUTING_TRACE_STORE_MESSAGE_TEXT` both ways through the real channel path: `message_text` absent when off, `message_sha256` present and identical either way. Also the `stages` **allowlist** (`routing_trace.SAFE_STAGE_FIELDS`) in both directions — that the sender-derived fields (`prompt`, `raw_response`, `llm_attempts[].error`) are neither stored nor served with the gate off, while the agent owner's own configuration (`candidates[].trigger_prompt`, `candidates[].prompt_examples`) is. Proved by driving a real classify and reading back through the admin API, never by asserting on the constant. Also the other side of that allowlist: `stages[].not_run_code` is both stored and served with the gate closed, where the `PASS_2_*_NOTE` sentence in `reason` it pairs with is neither — pinned in both directions on the `auto_install_off` arm. Also the **quoted message** (quote-aware routing): `quoted_message_text` / `quoted_message_author` — a third party's words — stored and served with the gate on, NULL at rest when captured with it off and withheld on read when read with it off, while `quoted_agent_id` is served either way and `message_sha256` stays the digest of the sender's own words. Driven by a real decision quoting another person's agent's reply, with the provider mocked at classifier depth so the prompt, raw response and reason genuinely hold the quote. Also the classifier's **categorical answer** (channel routing guidance, Phase 1): `stages[].intent` and `stages[].options[].ref_id` are stored and served with the gate closed, on both the read and write paths, from a real Pass 2 `clarify` reply over two catalog bundles whose off-ballot option carries the sender's words — and no stage field served with the gate closed carries them. |
...
| `routing_simulate_no_side_effects_test.py` | **The Phase 3 safety property, plus phase 6's channel-named simulate.** `POST /admin/routing/simulate` binds no thread, opens no session, installs nothing and sends nothing — each asserted against durable state, never a log, and each **mutation-checked** (see the module docstring; the channel-scoped form of the outbound assertion was found unfalsifiable and replaced). Plus the §12 conditions that make the exposure acceptable: superuser-only, audited naming both admin and target without the body, rate-limited, and a response identical to `GET /traces/{id}` except for exactly one extra key, `guidance_reply` (the reply the sender would have been shown; `None` for the routed decision this file simulates) — safe because it is composed only from ballot data the trace already serves under `stages[].candidates` or a fixed identity constant, never from sender text (guidance values themselves live in `routing_simulate_guidance_reply_test.py`). **Since phase 6 of the channels & identity unification it also owns the `channel_id` half**: naming a channel decides under that channel's *real* policy, so a simulate can finally put an **identity candidate on the ballot**, and naming none still cannot (`ResolvedChannelPolicy.for_no_channel()` holds `allow_identity_routing=False` on purpose — the absence of a channel is nobody's consent). The whole no-side-effects property is re-asserted with a channel named, the audit row records which channel the run decided under (and `None` when there was none), and a `channel_id` that resolves to nothing is refused with a 404 *before* the run spends an LLM call. |
| `routing_replay_and_recommendation_test.py` | `POST /traces/{id}/replay` (a new row, the original untouched, "nothing changed" said out loud, the outcome flip after a route is added, the 409 when the text gate is off) and `POST /traces/{id}/recommendation` (drafts without writing, scoped to this trace's candidates, no audit row). Also the shared per-admin rate-limit bucket across all three LLM-spending routes. Plus the quote-aware halves: a `quoted_reply` decision replays to the same agent without classifying, a classified quoted decision replays through the real renderer with the stored quote in the prompt (audit carries `quoted_chars`, not the body), the 409 still holds for a quoted trace, and a replay after the quoted agent is deleted (`quoted_agent_id` SET NULL) classifies and its diff reports the match-method, selection and removed-candidate change. |
| `routing_simulate_quoted_context_test.py` | Simulate's quote inputs (`quoted_message_text` ≤ 1000, `quoted_message_author` ≤ 120, `quoted_agent_id`): the quote section in the captured prompt with the User Message unchanged; `quoted_agent_id` on the target's ballot routes `quoted_reply` with no classifier, off the ballot it classifies and never becomes a candidate; an unknown `quoted_agent_id` is a 404 before any LLM call, trace or audit row; 422 one past each bound; blank text is no quote and an author alone is ignored; `CHANNEL_QUOTE_ROUTING_ENABLED=False` drops the quote and the preference (the audit's `quote_routing_enabled` says so); the audit carries `quoted_chars` / `quoted_agent_id`, never the body or the author; still no binding, session or send. Also a quoted agent deleted mid-decision (written straight to the row from inside the classifier call, the revocation test's documented shortcut) still leaves the trace, with `quoted_agent_id` NULL. |
| `routing_simulate_guidance_reply_test.py` | Channel routing guidance, Phase 2: a simulate the classifier answers `help` / `none` over a user's agents is `outcome="guided"` and returns `guidance_reply` (markdown, listing the ballot) on `RoutingSimulatePublic`, while sending nothing and opening no session; the stored trace read back through `GET /traces/{id}` carries no `guidance_reply` key but keeps `guidance_kind` / `guidance_options`. `CHANNEL_ROUTING_GUIDANCE_ENABLED=False` ⇒ `no_match` with `guidance_reply` null; a routed simulate and an empty ballot carry none either. The webhook half is `tests/api/server_channels/server_channels_routing_guidance_test.py`. |
| `routing_reachability_verdict_test.py` | **The Phase 4 headline.** The `diagnosis` on `GET /traces/{id}` — one server-authored sentence per branch, plus `?expected_agent_id=` for "why was THIS agent not a candidate". One test per verdict branch, each pinning the exact sentence (pinning only `code` would let the wording drift into saying something false). **Split by origin:** the channel half is driven through real simulate decisions; the App MCP half is driven from a `seed_routing_trace(origin="app_mcp")` row — not because nothing opens an App MCP capture (`AppMCPRoutingService.route_message` does, since phase 6), but because simulate runs the *channel* router, so reaching a real App MCP decision from here would mean standing up an MCP handler call for branches whose answer comes from the database rather than from the trace. The seed pins `ROUTING_TRACE_APP_MCP_MODE="full"` so it does not inherit the deployed default. **Channel routing guidance, Phase 4:** `guided_help` / `guided_clarify` / `guided_no_match`, each in its agent-list form (guidance on `pass_1`) and its bundle-list form (guidance on `pass_2`: a sender who owns nothing, answered over auto-install bundles), all from live simulates. A `clarify` simulate is named under a channel with notices and outbound credentials, since without one it routes the best pick. `clarified` on `routed` and on `parked_install` goes through **real webhook deliveries** (question, then "2"), because only a webhook writes the question row a reply is matched against. `clarified_unavailable` (a pick that settled `no_match`) is a race live, so it is seeded (`seed_routing_trace(match_method=...)`). The `expected_agent_considered` guided sentences are pinned too: listed among the reply's options, eligible but not listed, and the non-picked option on a clarified follow-up (no overlap score). `test_verdict_when_owned_agents_exist_but_the_classifier_matched_none` and `..._eligible_but_not_picked` pin the original `no_match` sentences with `CHANNEL_ROUTING_GUIDANCE_ENABLED=False`: the same `none` answer is `guided` with it on. Also the Jaccard near-miss ranking, including its going quiet with a notice rather than empty when the message-text gate is closed. Branches that need a candidate list on a non-live origin, and skip reasons no surface produces any more, live in `tests/unit/test_routing_reachability.py` alongside that service's unit-level properties. |
