...
platform agents from outside the platform, routed and bound per asker and conversation scope to a session. Threaded
spaces use the thread as scope; flat spaces use the space. Two people in the
same conversation never share a binding or an execution session.
...
| File | Covers |
|---|---|
| `server_channels_security_invariants_test.py` | Per-asker session isolation (active and failed bindings), independent routing and auto-installs for two people queued into one thread, same-asker pending-install races, flat-space resume across thread ids, and metadata-only quoted context when read access is missing. Also malformed JWTs (403, never 500) and the `critical_state` plan deviation. |
...
| `server_channels_stop_command_test.py` | The `/stop` channel control command and the acknowledgement it leans on. The command half: `interrupt_stream` called for the thread's own session with the text never ingested, the "nothing running" decline produced by the real `ValueError` path, a second asker's `/stop` never targeting the first asker's session (**invariant 6** — interception follows per-asker binding lookup and authorization), and a Chat retry acknowledged once via the `:control:` dedup key. The three shapes that are deliberately *not* a command: a first-message `/stop` (no binding, nothing to stop — it routes like any text), one with a file attached, and one on a `pending_install` binding, which must be answered and dropped rather than parked and replayed at the brand-new assistant. The acknowledgement half — four shapes of stopped turn that are **not** interchangeable: a partial answer keeps its text with the marker under it; a turn stopped **before the agent spoke** still says so (the likeliest stop there is, and the one that shipped as a stranded spinner); a relay that already sealed everything gets the marker as a fresh message **below** the sealed text (the mirror bug settled a bare marker over a live draft); and a thread that was never narrating is told **nothing** — the branch that also holds email at zero behaviour change. |
| `server_channels_quoted_context_test.py` | **Quote-aware routing** (`docs/plans/channel_quote_aware_routing_plan.md` §12.2 scenarios 1–10). The reported case: "can u?" quoting a human's "would be fun to hear a dad joke" classifies with the fenced quote section in the Pass-1 prompt while the User Message, `message_text` and the stored user message stay "can u?", and the agent's context shows the attributed snapshot instead of "text unavailable". A quote with no snapshot leaves the prompt byte-identical to an unquoted one. Quoting the sender's **own** agent's reply (the reply's id read off the delivery ledger after a real turn) routes by `quoted_reply` with the classifier stub raising, both candidates still listed and no prompt; the id alone is enough. A single-agent sender with a catalog on offer skips the probe. **Access never widens**: quoting another person's agent classifies over the sender's own ballot with no foreign or `identity:` candidate; a scope-excluded own agent stays skipped; a pin outranks the quote. The identity arm (Stage 2 takes the quoted binding without classifying, session in the owner's workspace, reply delivered) with its consent-off control. A parked message keeps its snapshot through `flush_pending_bindings`. `CHANNEL_QUOTE_ROUTING_ENABLED=False` removes the section and the preference but not the ingestion fallback. Email's half (scenario 11) is `server_channels_email_test.py::test_an_email_decision_renders_a_prompt_with_no_quote_section`. Shared setup lives in `tests/utils/channel_quote.py` (all four Chat verbs with real-shaped unique ids, provider capture at classifier depth, the ledger read). |
| `server_channels_routing_guidance_test.py` | **Guidance replies** (channel routing guidance, Phase 2): when the classifier answers `NONE` with `help` or `none`, the sender is told what they can reach instead of `REPLY_NO_MATCH`. `help` lists only the sender's own agents and the identities they may address (by full name or email local part, never the address), never another person's agent, an unreachable identity or a catalog bundle; a single-agent sender with a catalog on offer is classified and the unoffered bundle is recorded `pass_1_guided`; an empty ballot lists only admitted catalog bundles; `none` lists the sender's own ballot ahead of any catalog; Pass 1 `none` plus an unusable Pass 2 reply, the kill switch (`CHANNEL_ROUTING_GUIDANCE_ENABLED=False`) and an unusable reply all stay `REPLY_NO_MATCH`; `only_one` hands a meta question to the single agent with no model; `guidance_kind` / `guidance_options[].ref_id` survive the message-text gate on both paths; trace `guided`, debug feed `guided`; email gets `help` / `none` as plain text and is never asked a question (a `clarify` routes the best pick and stores no question row). `clarify` on Google Chat asks — see the next row. Composer unit half: `tests/unit/test_channel_routing_guidance.py`. |
| `server_channels_routing_clarification_test.py` | **The clarification round-trip** (channel routing guidance, Phase 3). A `clarify` answer on Google Chat settles the status notice into a numbered question, routes nowhere (trace `guided` / `guidance_kind="clarify"`) and stores one `channel_routing_clarification` row; "2" then ingests the **original** text and file into the second option's agent — never "2" itself — under `match_method="clarified"`, and the row is gone. A name reply resolves without a model and is not ingested. A short (≤ `CLARIFY_SELECTOR_MAX_WORDS`) free-text reply is read by the classifier over the options only, with the original as quoted context, and then delivered **after** the original on the binding it created; the same reply matched to no option routes as a new request, and one whose original then binds nothing (re-asked because the pick left the ballot) is dropped as `superseded` with the new question kept; a longer reply drops the question and routes fresh with no classifier call over the options. "No, thanks!" answers `REPLY_CLARIFY_CANCELLED` and ingests nothing. A classifier-read reply that loses to a plain "1" queued behind it records `already_answered` and is dropped (the synchronous claim's loser needs real concurrency and is not driven). An expired question is dropped lazily and the reply routes fresh; the flush tick purges expired rows only. A second asker in the same thread is unaffected. An identity option goes through Stage 2 into the owner's workspace; a bundle option parks an install of that bundle. An option policy no longer admits is never routed to (the original classifies over the current ballot). A redelivered answer is not ingested again (nor parked on a pending install, nor ingested once that install is flushed into a new session — its receipt survives `reset_for_new_session`); a redelivered original while the question is open is ignored. The kill switch asks nothing, stores no row and ignores a question already open; a Google Chat channel without outbound credentials is never asked either. With text storage off neither trace carries the sender's words. Open-question state is read and aged through `tests/utils/server_channel.py::get_routing_clarification` / `expire_routing_clarification` (Rule-1 exemptions, point 6 of that module's docstring). `resolve_choice` unit half: `tests/unit/test_channel_routing_resolve_choice.py`. |
...
- **No binding read API.** `ChannelThreadBinding` has no admin/user-facing GET
  ...
  reply text (`REPLY_STILL_SETTING_UP`, `REPLY_DENIED`,
...
- **Pass 2 needs a mocked classifier too.** The channel router (both passes)
  calls `AgentClassifier.classify_answer`, so that is the seam to patch
  (`app.services.routing.agent_classifier.AgentClassifier.classify_answer`) —
  `classify` is `classify_answer(...).result` and is what App MCP / Identity
  Stage 2 call. A patch there must return a `ClassifierAnswer`: use
  `tests.utils.routing.as_classifier_answer(classification(ref_id))` for a
  pick, `classifier_answer(intent="none"|"help")` for a model that answered
  `NONE` (over a non-empty ballot that is a guidance reply now), and
  `unusable_classifier_reply()` for an outage. `post_channel_message` /
  `patched_routing_externals` take the same via `classify_result=`,
  `classify_no_match=True` (intent `none`) and `classify_unusable=True`.
- **Only the same asker can lose a binding race.** The unique key is
  `(server_channel_id, scope_key, user_id)`. Two different askers each create
  their own binding, including when both auto-install the same bundle.
  `/stop` resolves the requesting asker's binding only; another person's first
  `/stop` is an ordinary opening message, never an interruption of an existing
  conversation member.
