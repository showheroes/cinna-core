...
- **Guidance Reply** — a templated (never model-authored) reply the channel router sends instead of a bare decline when the classifier's `intent` is `help` or `none`: a capped, escaped list of the sender's own ballot (their agents, reachable identities) or, when that ballot is empty, admitted catalog bundles. Gated by `CHANNEL_ROUTING_GUIDANCE_ENABLED`. See [Channel routing guidance](#channel-routing-guidance-help-no-match-and-clarifying-questions).
- **Clarifying Question** (`channel_routing_clarification`) — a one-row-per-asker state the router opens when the classifier's `intent` is `clarify` on a transport that can follow up: two or three near-equally-fit options, best pick first, with the sender's original message parked beside them. Answered by ordinal, name, or decline within `CHANNEL_ROUTING_CLARIFY_TTL_MINUTES`; unanswered, it expires and the next message routes fresh. See the same section.
...
### Channel routing guidance: help, no-match, and clarifying questions

The router's classifier (`AgentClassifier.classify_answer`, the shared prompt template every routing consumer renders) answers with a categorical `intent` — `route`, `clarify`, `help`, or `none` — alongside its pick. Every other routing consumer (App MCP Stage 1, identity Stage 2, both via `AgentClassifier.classify`) still only ever acts on the pick and treats `help`/`none` alike as "nothing to route" — see [App MCP Server](../app_mcp_server/app_mcp_server.md#routing-priority) and [Identity Routing](../identity_routing/identity_routing.md#stage-2-routing-priority) for the one accepted behavioural change this brings to those two surfaces: a bare meta question ("what can you do?") no longer gets best-picked, it declines. A help question that also carries a task ("help me write a post") is a task, not help — that is the model's call to make, not a keyword match, and the prompt says so explicitly.

Only the channel router turns `intent` into a reply of its own (`channel_routing_guidance.py`), gated by `CHANNEL_ROUTING_GUIDANCE_ENABLED` (default on):

- **`help`** — the sender asked what the assistant can do. The reply lists their own post-policy ballot: owned agents in scope, plus reachable identities when `allow_identity_routing` is on. An identity entry always carries the fixed description "passes your message on to one of their assistants" and shows the owner's name or, failing that, the email local part only — never the raw address. When that ballot is empty (a freshly auto-registered sender with nothing of their own and identity routing off or empty), the reply falls back to the admitted catalog bundles instead — the same visibility check every install path uses, so a bundle nobody can install is never listed.
- **`none`** — nothing fit. The reply is the same shape as `help`'s (the ballot, or bundles if the ballot is empty), so a genuine miss and "what can you do" read as siblings from the sender's side. The old flat "couldn't find an agent for that" decline (`REPLY_NO_MATCH`) now only appears when there is genuinely nothing to list on either side.
- **Conservative, never a bare shrug.** Pass 1 answering `none` and Pass 2 then failing to produce anything usable — an outage, a non-JSON reply, an empty catalog — settles as `REPLY_NO_MATCH`, not a half-built guidance reply.
- **Never model prose.** The composer (`compose_guidance_reply`) is a pure function over plain ballot data — ids, names, trigger prompts already on the ballot the sender's own resolved policy built — never the sender's message, the model's `reason`, or a rewritten task. Names and descriptions are collapsed to one line, clamped, and have markdown/link/mention characters escaped before they reach the reply. The list is capped at `CHANNEL_ROUTING_GUIDANCE_MAX_LISTED` (default 5) with a trailing "…and N more."
- **The reply settles the status notice** exactly the way `REPLY_NO_MATCH` always has — it is the answer, and it stays.
- **A pin or the `only_one` short-circuit still skip all of this** — no classifier call means no `intent`.
- **Trigger prompts are now something a real person reads.** A `help` or `none` reply shows the sender the trigger prompt of every agent it lists — see [Agent Prompts — Trigger Prompt Scope](../../agents/agent_prompts/agent_prompts.md#trigger-prompt-scope) for the consequence for anyone writing one.

**Clarifying questions.** When `intent` is `clarify`, the router only actually **asks** the sender to choose where the channel can follow the question up — the resolved transport supports a status notice, its outbound credentials are configured, and guidance is on (Google Chat qualifies today; email, and a channel with no outbound credentials configured, route to the classifier's best pick instead, exactly as before this feature).

- One open question per `(channel, scope_key, asker)` — a second asker in the same conversation is unaffected, and asking again replaces the row. Table `channel_routing_clarification`; deliberately not a `ChannelThreadBinding` — a question is not yet a conversation. Holds the two or three options (best pick first) and the sender's original message in the same shape a parked message holds it (files by id, kept `temporary`), and expires after `CHANNEL_ROUTING_CLARIFY_TTL_MINUTES` (default 30) — lazily, on the sender's next message, and swept by the same pending-binding scheduler tick that flushes parked messages.
- **Answering.** A short reply (at most `CLARIFY_SELECTOR_MAX_WORDS`, 4, words after filler) is read as a choice first: an ordinal ("2", "#2", "option 2"), an exact/prefix/unique-substring match on a listed name (3+ characters, at most 2 extra words), or a decline ("no", "no thanks", "no thank you", "nope", "neither", "none", "cancel"), which cancels the question outright (`REPLY_CLARIFY_CANCELLED`). A reply that negates the option it names — `not`, `don't`, `isn't`, `never`, `except`, or `no` before anything but a number — is never read as choosing it (`resolve_choice`'s `_negates`); it falls through to the classifier call below instead, while `no 2` still picks option 2 and a bare `no` / `no thanks` still cancels. A reply that reads like neither a choice nor a decline, but still fits that word budget, gets one extra classifier call over just the two or three options with the original message as quoted context — a pick there routes the **original** message and delivers the reply right after it on the same binding (the reply is dropped and recorded `superseded` if the original itself failed to bind anything); a `none` from that call routes the reply as a fresh request. A reply longer than the word budget is read as a new request outright — the question is dropped with no extra classifier call, and the reply routes normally.
- **The chosen option is re-validated, never trusted from the stored row** — it must still be on the current ballot, or classification runs fresh as if nothing were asked. An agent choice re-loads and re-checks ownership; an identity choice hands off to Stage 2; a bundle choice becomes a parked install. The resulting decision carries `match_method="clarified"`. That fresh classification never earns a second question of its own — `ChannelRoutingService.decide` forces `can_clarify` off for the rest of the decision whenever `chosen_ref_id` is set, since a sender who just answered one question has no reason to expect another.
- **Only one answer wins.** Two replies racing to answer the same question both resolve; the loser is recorded `already_answered` in the debug feed and produces no second routing effect, even when its own classifier call read no option — it is dropped rather than routed as a new request, because the winner is already routing the original on the same scope key and a fresh route beside it would risk a second reply or a binding race. A redelivery of the answer itself is receipted and ignored (the receipt survives the new session's context reset); a redelivery of the *original* message while the question is still open is likewise ignored.
- **An answer carrying attachments is never read as a choice** — files are content for an agent, so it is a new request, and the open question is dropped rather than swallowing them.
- **Flag off, or the transport can't follow up:** none of this runs. A `clarify` intent is treated exactly as `route` — the classifier's best pick is used — and no `channel_routing_clarification` row is ever written.

See [Server Channels — Technical Reference](server_channels_tech.md#channel-routing-guidance-and-clarification) for the module split and schema, and [Auto Routing Tuning](../routing_tuning/routing_tuning.md) for how `intent`, the guidance shape, and the clarified follow-up land on the trace and the reachability verdict.
