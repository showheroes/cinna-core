...
### Backend — Framework
- `backend/app/services/agents/command_service.py` — Core framework: `CommandContext`, `CommandResult`, `CommandHandler` (ABC), `CommandService` (static registry); includes `include_in_llm_context` class attribute and `CommandService.get_handler()` method
...
- `CommandService.get_handler(name)` — Returns the registered handler for a given command name, or `None` if not registered; used by `build_non_llm_prefix` to look up `include_in_llm_context`
