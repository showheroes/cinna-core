...
Agents can expose a set of named shell commands that users and A2A clients invoke directly — without spending tokens on an LLM turn. These commands power the `/run:<name>` slash command in the chat UI, surface as A2A skills (`cinna.run.<name>`) in the authenticated agent card, appear in the slash-command autocomplete popup with a tooltip showing the resolved shell string, and feed output back into the LLM context on the next conversation turn.
...
### Invoking `/run` Without Arguments

Typing `/run` with no command name returns a formatted markdown table of all declared commands — name, description, and whether the command produces streaming output. This serves as an in-chat discovery mechanism and is the convention A2A LLM-clients use to enumerate available commands before invoking them.
...
|---------|-------------|-----------|
| `check` | Monthly data-quality check. Run after the month closes. | no |
| `report` | Generate the weekly report and upload it to S3. | no |
```
...
`/app/workspace/docs/CLI_COMMANDS.yaml` (relative to workspace root: `docs/CLI_COMMANDS.yaml`)
...
2. **Post-action** — after every session stream (`STREAM_COMPLETED`, `STREAM_ERROR`) and after every scheduled execution (`CRON_COMPLETED_OK`, `CRON_TRIGGER_SESSION`, `CRON_ERROR`).
3. **After rebuild** — after a successful `/rebuild-env` operation completes.
4. **Explicit `/run` (no args)** — forces a cache refresh when the user sends `/run` without a command name.
...
The `GET /api/v1/sessions/{session_id}/commands` endpoint appends dynamic `/run:<name>` entries from the cache to the static command list. Each dynamic entry includes `resolved_command` — the raw shell string — which the `SlashCommandPopup` renders as a tooltip on hover.

### Availability

Dynamic entries are `is_available=false` when the environment is not in `running` status, because execution requires a live container. The tooltip still shows the resolved command for reference.
...
4. The backend flushed events to the DB every 2 seconds. A system message is finalized at completion with exit code, duration, and accumulated output.

### Output Display

The resulting system message is rendered in the chat as a terminal-style block:
- **Header**: command name and truncated resolved command string (with a copy button)
- **Output area**: scrollable monospace block with stdout (default) and stderr (amber)
- **Footer**: exit code badge (green check / red X), timed-out / interrupted / truncated labels if applicable
