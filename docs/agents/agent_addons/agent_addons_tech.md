...
| `credential_issues` | `list[AddonCredentialIssuePublic]` | `[]`. One `{slot, type, reason}` per credential slot that is not usable yet — for a catalog row the slots of its pinned revision, for any other non-orphan row the slots its skills declare (deduplicated by slot and type) — `reason` ∈ `not_linked` / `not_configured` / `access_revoked`. The client deep-links each one to the agent's Credentials tab |
| `orphan` | `bool` | `False` |
...
#### `credential_issues` — the revision for catalog rows, `SKILL.md` for the rest

`project()` builds one `SkillSlotIndex.build_for_agent(session, agent,
with_declarations=…)` — only when the agent has a `source=catalog` link or a
non-catalog index entry declares credentials, and `with_declarations` is set in
the second case. A catalog row's issues come from
`slot_index.issues_for_link(link.id)`, which reads the **frozen specs of the
revision each link pins**, never the environment's skill index, so its status is
identical for a pre-feature container and a fresh one.

Every other non-orphan row — a local skill, a marketplace or bundle plugin — is
judged once its skills are folded in, on the `(slot, type)` pairs those skills
declare (deduplicated, so two skills of one plugin sharing a slot give one
issue), through `slot_index.issues_for_declarations`. Its `SKILL.md` is the only
record of those declarations; a pre-feature container reports none and the row
stays silent. Either way the credential half costs a constant number of queries
whatever the number of rows (see
